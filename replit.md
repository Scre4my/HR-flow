# HR Survey - Система опросов

## Обзор
Веб-сервис для создания и проведения онлайн-опросов, ориентированный на нужды HR-отдела компании. Проект включает админ-панель для создания опросов, публичные страницы для прохождения и систему анализа результатов.

## Архитектура

### Backend (Symfony 7)
- **Расположение**: `/backend`
- **Фреймворк**: Symfony 7 с Doctrine ORM
- **База данных**: PostgreSQL
- **API**: RESTful API

#### Структура сущностей:
- `Survey` - опросы (название, описание, статус, тип опроса, страницы, логика переходов)
- `Question` - вопросы (текст, тип, варианты ответов, номер страницы)
- `Response` - ответы респондентов
- `Answer` - конкретные ответы на вопросы

#### Типы опросов:
- `single_page` - одностраничный (все вопросы на одной странице)
- `multi_page` - многостраничный (вопросы распределены по страницам с логикой переходов)

#### Типы вопросов:
- `text` - текстовый ответ
- `single_choice` - выбор одного варианта
- `multiple_choice` - множественный выбор
- `rating` - шкала оценки 1-10
- `radio_text` - один вариант + текстовое поле для комментария
- `checkbox_text` - множественный выбор + текстовое поле для комментария

### Frontend (React + TypeScript)
- **Расположение**: `/frontend`
- **Сборка**: Vite
- **Графики**: Recharts
- **Маршрутизация**: React Router

#### Страницы:
- `/admin` - список опросов
- `/admin/surveys/new` - создание опроса (пошаговый мастер)
- `/admin/surveys/:id/edit` - редактирование опроса
- `/admin/surveys/:id/results` - результаты опроса
- `/survey/:id` - публичная страница прохождения опроса
- `/thank-you` - страница благодарности

#### Мастер создания опроса (3 шага):
1. **Создание вопросов** - добавление вопросов с выбором типа
2. **Выбор типа опроса** - одностраничный или многостраничный
3. **Настройка страниц** (для многостраничных) - распределение вопросов по страницам и логика переходов

## API Endpoints

### Опросы
- `GET /api/surveys` - список всех опросов
- `GET /api/surveys/active` - активные опросы
- `GET /api/surveys/:id` - детали опроса с вопросами
- `POST /api/surveys` - создание опроса
- `PUT /api/surveys/:id` - обновление опроса
- `DELETE /api/surveys/:id` - удаление опроса
- `POST /api/surveys/:id/toggle` - переключение статуса

### Ответы
- `POST /api/surveys/:id/submit` - отправка ответов
- `GET /api/surveys/:id/responses` - список ответов
- `GET /api/surveys/:id/statistics` - статистика по опросу

## Запуск

Проект запускается через `bash start.sh`:
- Backend: PHP built-in server на порту 8000
- Frontend: Vite dev server на порту 5000

## Технологии
- PHP 8.2
- Symfony 7.1
- Doctrine ORM
- Node.js 20
- React 18
- TypeScript
- Vite
- Recharts
- PostgreSQL

## Последние изменения
- 2026-01-30: Добавлена поддержка многостраничных опросов и новых типов вопросов
  - Вопросы отображаются на своих страницах при прохождении многостраничного опроса
  - Добавлен индикатор прогресса для многостраничных опросов
  - Новые типы: радио+текст (radio_text) и чекбокс+текст (checkbox_text)
  - Логика условных переходов между страницами
- 2026-01-29: Добавлен пошаговый мастер создания опросов
  - Шаг 1: Создание вопросов
  - Шаг 2: Выбор типа опроса (одностраничный/многостраничный)
  - Шаг 3: Настройка страниц и логики переходов (для многостраничных)
  - Поддержка условных переходов между страницами
- 2025-12-11: Создан полный MVP проекта
  - Symfony backend с RESTful API
  - React frontend с админ-панелью
  - Система создания опросов с 4 типами вопросов
  - Публичная страница прохождения опросов
  - Визуализация результатов (графики)
